...
ssh -o "UserKnownHostsFile=/dev/null" -o StrictHostKeyChecking=no $agent_user@$gateway_ip -i ~/.ssh/access-agent-key -T "bash /tmp/create-tunnel-from-agent-to-compute.sh $local_ip $vlan_id $public_key"