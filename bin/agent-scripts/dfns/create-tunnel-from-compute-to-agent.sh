...
ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no $agent_user@$gateway_ip -i ~/.ssh/access-agent-key -T "bash $script_name $local_ip $vlanID \"$public_key\""
